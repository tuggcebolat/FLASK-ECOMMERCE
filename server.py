...
HEROKU=False
if(not HEROKU):
    os.environ['DATABASE_URL']="dbname='postgres' user='postgres' host='localhost' password=postgres"
...
def movie_detail_page(id):
    movie=select("id,name","movie","id={}".format(id),asDict=True)
...
def actor_detail_page(id):
    actor=select("id,name","movie","id={}".format(id),asDict=True)
